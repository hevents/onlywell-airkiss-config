...
2. **本仓库 APK（推荐）**：安装 [`OnlywellAirkiss-Config-v1.2.apk`](https://github.com/hevents/onlywell-airkiss-config/releases/download/v1.2/OnlywellAirkiss-Config-v1.2.apk)（点链接直接下载；或到 [Releases 页](https://github.com/hevents/onlywell-airkiss-config/releases) 查看）→ 连 **2.4G** WiFi（App 会自动填 SSID，连 5G 会警告）→
...
10. **WiFi 刷新（v1.1 起，v1.2 完善动画）**：打开时连的是 5G？去系统设置切到 2.4G 后回到应用即**自动刷新**当前 WiFi（SSID + 频段提示同步更新）。页面上部**下拉**可手动刷新：拖拽时刷新头部**跟手展开**，拉过阈值提示「松开立即刷新」，**松手才触发**（转圈「正在刷新…」，完成后平滑收起；未达阈值松手则收起不刷新）。手动改过 SSID 输入框则刷新不覆盖用户输入，只更新频段提示行。
